...
def compute_metrics(hr_tensor: torch.tensor,
    ...
    for start in tqdm.tqdm(range(0, total, batch_size)):
        ...
        for idx in range(batch_score.size(0)):
            ...
            gold_neighbor_ids = all_triplet_dict.get_neighbors(cur_ex.head_id, cur_ex.relation,cur_ex.time)
        ...
        raw_scores.extend(batch_score[:, :].tolist())
    ...
    assert len(raw_scores) == total
    return topk_scores, topk_indices, metrics, ranks,raw_scores
...
def eval_single_direction(predictor: BertPredictor,
    ...
    topk_scores, topk_indices, metrics, ranks,raw_scores = compute_metrics(hr_tensor=hr_tensor, entities_tensor=entity_tensor,
                                                                target=target, examples=examples,
                                                                batch_size=batch_size)
    eval_dir = 'head' if eval_forward else 'tail'
    ...
    for idx, ex in enumerate(examples):
        ...
        pred_info = PredInfo(head=ex.head, relation=ex.relation,time=ex.time,
                       ...
                       #      hr_tensor=hr_tensor[idx].tolist()
                        )
