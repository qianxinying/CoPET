...
def construct_mask(row_exs: List, col_exs: List = None) -> torch.tensor:
    ...
    for i in range(num_row):
        head_id, relation, time = row_exs[i].head_id, row_exs[i].relation,row_exs[i].time
        neighbor_ids = train_triplet_dict.get_neighbors(head_id, relation,time)
...
def construct_self_negative_mask(exs: List) -> torch.tensor:
    ...
    for idx, ex in enumerate(exs):
        head_id, relation, time = ex.head_id, ex.relation, ex.time
        neighbor_ids = train_triplet_dict.get_neighbors(head_id, relation,time)
        if head_id in neighbor_ids:
            mask[idx] = 0
    return mask.bool()

def construct_negative_mask(exs: List) -> torch.tensor:
    mask = torch.ones(len(exs))
    for idx, ex in enumerate(exs):
        head_id, relation, time = ex.head_id, ex.relation, ex.time
        negatives = ex.negative
        neighbor_ids = train_triplet_dict.get_neighbors(head_id, relation,time)
        if negatives in neighbor_ids:
            mask[idx] = 0
    return mask.bool()